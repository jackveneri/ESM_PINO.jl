...
# Fix: buffer is just autoregressive_steps, not 2*steps*dt
qg3ppars, qg3p, S, solψ, solu = ESM_PINOQG3.load_precomputed_data(
    root=root, N_sims=N_sims + autoregressive_steps + N_val, res=res, gpu=gpu)

velocity = ESM_PINOQG3.prepare_velocity_ML_data(solψ, qg3p, gpu=gpu)
sol = cat(solu, solψ; dims=3)

q_0_train, q_evolved_train, q_0_val, q_evolved_val, μ, σ, _ = ESM_PINOQG3.preprocess_data(
    sol, normalize=true, to_gpu=gpu, dt=dt, channelwise=true, 
    train_fraction=N_sims/(N_sims+N_val))
...
trained_model = ESM_PINOQG3.train_RKSFNO_model(
    q_0_train, q_evolved_train, q_0_val, q_evolved_val, qg3ppars,
    ...
    use_norm=true,
    inner_skip=true,
    outer_skip=false,       # Fix: must be false for RKSFNO
    use_physics=true,
    geometric=true,
    spectral=false,
...
# Fix: slice q_0 to match autoregressive_target's batch dimension
N_ar_train = size(autoregressive_target, 4)
N_ar_val   = size(autoregressive_target_val, 4)

fine_tuned_model = ESM_PINOQG3.fine_tuning(
    q_0_train[:, :, :, 1:N_ar_train], autoregressive_target,
    q_0_val[:,   :, :, 1:N_ar_val],   autoregressive_target_val,
    trained_model_architecture, trained_model_ps, trained_model_st,
    parameters=pars,
    use_physics=false,
    geometric=true,
...
@save joinpath(root, "models/PIRKSFNO_results.jld2") model ps st dt N_sims μ σ res
