...
"""
function ESM_PINO.SphericalKernel(hidden_channels::Int, pars::QG3.QG3ModelParameters, activation=NNlib.gelu; modes::Int=pars.L, batch_size::Int=1, gpu::Bool=true, zsk::Bool=false) 
    ...
    spherical = ESM_PINO.SphericalConv(pars, hidden_channels; modes=modes, batch_size=batch_size, gpu=gpu, zsk=zsk)
    return ESM_PINO.SphericalKernel(conv, spherical, activation)
end
"""
$(TYPEDSIGNATURES)
...
- `activation::F`: Elementwise activation function
"""
function ESM_PINO.SphericalKernel(hidden_channels::Int, ggsh::QG3.GaussianGridtoSHTransform, shgg::QG3.SHtoGaussianGridTransform, activation=NNlib.gelu; modes::Int=ggsh.output_size[1], zsk::Bool=false)
    conv = Lux.Conv((1,1), hidden_channels => hidden_channels, pad=0, cross_correlation=true, init_weight=kaiming_normal, init_bias=zeros32)
    spherical = ESM_PINO.SphericalConv(hidden_channels, ggsh, shgg, modes, zsk=zsk)
    return ESM_PINO.SphericalKernel(conv, spherical, activation)
end
