...
trained_model, loss_function = train_model(q_0, q_evolved; seed=0, maxiters=maxiters, hidden_channels=hidden_channels, parameters=par)
...
for ilvl in 1:3
    ...
    CairoMakie.record(fig_pred, joinpath(root, "FNO_PINO_prediction_fps20_sf_lvl$(ilvl).gif"), plot_times;
        framerate=20) do it
        # Update observable data
        data_obs_pred[] = permutedims(sf_plot_pred[ilvl,:,:,it],(2,1))
        ax_pred.title = @sprintf("FNO PINO Prediction at time = %d - %.2f d", it, it * qg3p.p.time_unit)
    end
    ...
    CairoMakie.record(fig_err, joinpath(root, "FNO_PINO_error_fps20_sf_lvl$(ilvl).gif"), plot_times;
        framerate=20) do it
        # Update observable data
        data_obs_err[] = permutedims(mistake[ilvl,:,:,it],(2,1))
        ax_err.title = @sprintf("FNO PINO Error at time = %d - %.2f d", it, it * qg3p.p.time_unit)    
    end
...
@save joinpath(root, "FNO_PINO_results.jld2") sf_plot_pred sf_plot_evolved mistake loss ps st 
