    ...
    config_name = "params=$(param_set)_ch=$(hidden_channels)_modes=$(modes)_batch=$(batch_size)_gpu=$(use_gpu)_zsk=$(operator_type)"
        @testset "Configuration: config_name" begin 
            ...
            @testset "Initialization" begin
                ...
                layer = SFNO_Block(hidden_channels,qg3ppars; 
                ...
                @test haskey(ps, :spherical_kernel)
                
                # Verify state is properly initialized (should be empty NamedTuple)
                @test haskey(st, :channel_mlp)
                @test haskey(st, :spherical_kernel)
                ...
                layer_direct = SFNO_Block(hidden_channels, ggsh, shgg, modes=modes, operator_type=operator_type)
                
                ps_direct, st_direct = Lux.setup(rng, layer_direct)
                 # Verify parameters exist and have correct structure
                @test haskey(ps_direct, :channel_mlp)
                @test haskey(ps_direct, :spherical_kernel)
                
                # Verify state is properly initialized (should be empty NamedTuple)
                @test haskey(st_direct, :channel_mlp)
                @test haskey(st_direct, :spherical_kernel)
            end
